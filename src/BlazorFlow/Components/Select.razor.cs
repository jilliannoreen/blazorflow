...
public partial class Select<TItem, TValue>
{
    ...
    [Parameter] public bool EnableFloatingLabel { get; set; } = false;
    ...
    private string? PlaceholderValue => EnableFloatingLabel ? " " : Placeholder;
    ...
    private string LabelClass => ClassBuilder
        .Default(!EnableFloatingLabel
            ? "block mb-1 text-[10px]/4 font-normal text-gray-500" // static label above input
            : "absolute left-[16px] text-sm text-gray-600 transition-all z-10 origin-[0] scale-75 cursor-text px-1.5") // floating style
        .AddClass("bg-white", Variant == Variant.Outlined && EnableFloatingLabel)
        .AddClass(GetLabelPositionClass(), EnableFloatingLabel) // only for floating label positioning
        .AddClass("peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0", EnableFloatingLabel)
        .AddClass("peer-focus:scale-75", EnableFloatingLabel)
        .AddClass(GetLabelColorClass())
        .Build();
    ...
    private string GetIconClass => ClassBuilder
        .Default("absolute top-1/2")
        .AddClass("-translate-y-1/2", EnableFloatingLabel)
