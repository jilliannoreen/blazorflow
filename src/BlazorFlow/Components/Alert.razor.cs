...
public partial class Alert : ComponentBase
{
    ...
    private string AlertClass => ClassBuilder
        .Default("rounded-lg p-4 flex flex-wrap items-center gap-3")
    ...
    {
        AlertType.Info => "bg-(--blue-50) text-(--blue-700)",
        AlertType.Success => "bg-(--green-100) text-(--green-800)",
        AlertType.Warning => "bg-(--amber-200) text-(--amber-800)",
        AlertType.Danger => "bg-(--red-100) text-(--red-500)",
        _ => ""
    };
