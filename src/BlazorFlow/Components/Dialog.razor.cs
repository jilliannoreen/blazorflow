...
public partial class Dialog : ComponentBase, IAsyncDisposable 
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            ...
            // Register this instance’s Id to its DialogReference
            DialogReference?.SetDialogId(Id);
        }
    }
    ...
    /// </summary>
    [Parameter]
    public string Id { get; set; } = $"dialog-{Guid.NewGuid()}";
...
public class DialogReference
{
    ...
    private readonly Action<DialogReference> _closeCallback;
    ...
    public DialogReference(Action<DialogReference> closeCallback)
    {
        _closeCallback = closeCallback;
    }

    internal void SetDialogId(string id) => Id = id;
