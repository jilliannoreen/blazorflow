...
public class DrawerService : IDrawerService
{
    ...
    {
        await _js.InvokeVoidAsync("blazorFlowInterop.drawer.init", id, options);
    }
    
    /// <inheritdoc />
    public async Task ShowAsync(string id)
    {
        await _js.InvokeVoidAsync("blazorFlowInterop.drawer.show", id);
    }
    /// <inheritdoc />
    public async Task HideAsync(string id)
    {
        await _js.InvokeVoidAsync("blazorFlowInterop.drawer.hide", id);
    }
    /// <inheritdoc />
    public async Task ToggleAsync(string id)
    {
        await _js.InvokeVoidAsync("blazorFlowInterop.drawer.toggle", id);
    }
    /// <inheritdoc />
    public async Task<bool> IsVisibleAsync(string id)
    {
        return await _js.InvokeAsync<bool>("blazorFlowInterop.drawer.isVisible", id);
    }
    /// <inheritdoc/>
